    ...
    while (1) {
        ...
        case STATE_START:
            if (c == '\0') {
                ...
                if (tokenlist_add(list, TOK_SEMI, ";") < 0) goto error;
                p++;
            } else if (c == '&') {
                if (*(p + 1) == '&') {
                    if (tokenlist_add(list, TOK_AND, "&&") < 0) goto error;
                    p += 2;
                } else {
                    /* Single & not supported, treat as error for now */
                    goto error;
                }
            } else if (c == '|') {
                if (*(p + 1) == '|') {
                    if (tokenlist_add(list, TOK_OR, "||") < 0) goto error;
                    p += 2;
                } else {
